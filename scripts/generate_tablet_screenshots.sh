...
mkdir -p listing/android-tablet
...
if [ -z "$1" ]; then
  ...
  if [ -z "$DEVICE_ID" ]; then
    ...
    echo "💡 Tablet screenshot generation is only supported on emulators to ensure consistent framing."
...
echo "🧹 Cleaning tablet output directory..."
rm -rf listing/android-tablet/*.png

echo "🎬 Starting automated tablet screenshot generation..."

flutter drive \
  --driver=test_driver/screenshot_driver.dart \
  --target=integration_test/screenshot_test.dart \
  --dart-define=DEVICE_TYPE="tablet" \
  -d "$DEVICE_ID"

if [ $? -eq 0 ]; then
  echo "✅ Tablet screenshots generated successfully in listing/android-tablet/"
  ls -lh listing/android-tablet/
else
  echo "❌ Error: Tablet screenshot generation failed!"
  exit 1
fi
